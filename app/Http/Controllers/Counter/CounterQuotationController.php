...
class CounterQuotationController extends Controller
{
    use ResponseTraits;
    public function index(Request $request)
    {
        ...
        $quotations = DB::table('quotations')
            ...
            ->get(); // Fetch quotations from the database
        // dd($quotations); // Debugging line to check the fetched quotations
        return view('Counter.quotation', compact('quotations'));
    }
    public function create(Request $request)
    {
        ...
        return view('Counter.Model.quotationmodel', compact('items', 'branch_id', 'quotation', 'purchse_items'));
    }
    ...
    // Add this method to allow adding items to the cart using only item details (not saleorder id)
    public function addToCart(Request $request)
    {
        // Validate the required quotation_id
        $validation = Validator::make($request->all(), [
        ...
        // Fetch all item_ids for the given quotation_id
        $priceIds = DB::table('quotation_items')
            ->where('qout_id', $request->quotation_id)
            ->whereNull('deleted_at')
            ->pluck('price_id')
            ->toArray();
        ...
        // Build the query string as item_id=[id1,id2,id3]
        $query = 'price_ids=[' . implode(',', $priceIds) . ']';
        $redirectUrl = url('home') . ($query ? '?' . $query : '');
