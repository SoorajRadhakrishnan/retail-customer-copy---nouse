...
class OrderTypeWiseExport implements FromCollection,WithHeadings, WithEvents, WithCustomStartCell, WithMapping
{
    protected $request;
    ...
    */
    public function collection()
    {
        ...
        if(auth()->user()->branch_id)
        {
            return  SaleOrders::leftJoin('sale_order_items', function ($join) {
                ...
                $query->where('sale_orders.order_type',$order_type);
                ...
                ->groupBy('sale_orders.order_type')
                ->select(DB::raw('sale_orders.order_type,sum(sale_order_items.price * sale_order_items.qty) as total_price, sum(sale_order_items.item_unit_price * sale_order_items.qty) as after_discount'))
                ->get();
        }
        else{
            if(getSessionBranch()){
    ...
    public function map($user): array
    {
        ...
        return [
            Str::ucfirst(str_replace('_',' ',$user->order_type)),
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->setCellValue('A1', 'Order Type Report');
