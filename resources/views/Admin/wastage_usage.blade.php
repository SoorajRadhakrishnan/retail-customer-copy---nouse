                                    ...
                                    <div class="row">
                                        <div class="col-md-2">
                                            ...
                                            <select class="form-control rounded-10 select2" id="item_type" name="item_type"
                                                ...
                                                <option value="1" {{ $item_type == 1 ? 'selected' : '' }}>Salelable Item</option>
                                                <option value="2" {{ $item_type == 2 ? 'selected' : '' }}>Raw Material</option>
                                            </select>
                                        </div>
                                        <div class="col-md-3">
                                            ...
                                            <select class="form-control rounded-10 select2" id="item_id" name="item_id"
                                                ...
                                                ?>
                                                <option value="{{ $item_id->item_id }}"
                                                    @if ($item_id->price_id == $price_id) selected="selected" @endif>
                                                    {{ $item_id->item_name }}{{ $item_id->size_name ? ' - ' . $item_id->size_name : '' }}
                                                </option>
                                            @endforeach
                                            </select>
                                        </div>
                                                ...
                                                @if (count($items) > 0)
                                                    @foreach ($items as $key => $item)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <input type="hidden" name="price_id[]" value="{{ $item->id }}"> <!-- Ensure you use the correct field here -->
                                                            <input type="hidden" name="branch_id[]" value="{{ $item->branch_id }}">
                                                            <input type="hidden" name="stock[]" value="{{ $item->stock }}">
                                                            <td>{{ Str::ucfirst($item->item->item_name ?? 'N/A') }}</td><!-- Item Name -->
                                                            <td>{{ $item->stock }}</td> <!-- Available Stock -->
                                                            <td>
                                                                <input type="number" name="wastage_qty[]" class="form-control w-auto rounded-10" placeholder="Enter Wastage">
                                                            </td> <!-- Wastage -->
                                                            <td>
                                                                <input type="number" name="usage_qty[]" class="form-control w-auto rounded-10" placeholder="Enter Usage">
                                                            </td> <!-- Usage -->
                                                        </tr>
                                                    @endforeach
                                                @endif
