...
$title = 'CUSTOMER-OUTSTANDING'.$branch_name;
                ...
                <div class="row d-flex justify-content-center">
                    <div class="col-xl-7 col-lg-9">
                        <div class="az-dashboard-one-title">
                            <div>
                                <h2 class="az-dashboard-title">Customer - Outstanding Report</h2>
                            </div>
                                        ...
                                        @if (count($data) > 0)
                                            @foreach ($data as $key => $value)
                                                <tr>
                                                    ...
                                                    @endif
                                                    <td>{{ getCustomerDetById($value->customer_id) }}</td>
                                                    <td>{{ showAmount($value->credit - $value->debit) }}</td>
                                                </tr>
                                                <?php
                                                    $total_amount += $value->credit - $value->debit;
                                                ?>
                                            @endforeach
                                        @endif
                                    </tbody>
