    ...
    <div class="col-12 p-0">
            ...
            <div class="row">
                ...
                <div class="col-3 pr-1">
                    <div class="form-group mt-0 mb-0">
                        ...
                        <input type="date" class="form-control rounded-10" id="quotation_date" name="quotation_date"
                            required
                            value="{{ old('quotation_date', optional($quotation)->date_added ?? now()->format('Y-m-d')) }}">
                        <div class="valid-feedback">&nbsp;</div>
                        <div class="invalid-feedback">&nbsp;</div>
                    </div>
                </div>
                            ...
                            <select class="form-control w-auto rounded-10 select2 addITem" id="items" name="items"
                                ...
                                @foreach ($items as $item)
                                    ...
                                    <option value="{{ $item->price_id }}" data-price_id="{{ $item->price_id }}"
                                        ...
                                        data-item_size="{{ $item->size_name }}" data-unit_price="{{ $item->price }}"
                                        data-image="{{ $item->image }}">{{ $item->item_name . $item->size_name }}
                                    </option>
                                @endforeach
                            </select>
                            {{-- <div class="input-group-append">
                ...
                {{-- <div class="col-6">
                    <div class="form-group mt-0 mb-0">
                        <label class="mb-0 font-weight-bold">Select Item</label>
                        <div class="input-group">
                            <input type="number" class="form-control rounded-10" id="item_name" name="item_name"
                        ...
                        @if ($purchse_items != null)
                            @foreach ($purchse_items as $key => $pur_item)
                                <tr class="tr{{ $key + 1 }}">
                                    ...
                                    <td class="t-1">
                                        @if ($pur_item->image)
                                            <img src="{{ url('storage/item_image/' . $pur_item->image) }}"
                                                alt=""
                                                style="width:32px;height:32px;object-fit:cover;margin-right:6px;border-radius:4px;">
                                        @else
                                            <img src="{{ url('assets/img/placeholder1.png') }}" alt=""
                                                style="width:32px;height:32px;object-fit:cover;margin-right:6px;border-radius:4px;">
                                        @endif
                                    </td>
                                    ...
                                    <td class="t-1">
                                        <input type="number" class="form-control rounded-10 cost-price-blurred"
                                            placeholder="" name="unit_price[]" required="" autofocus=""
                ...
                <div class="form-group mt-0 mb-0 col-3">
                    ...
                    <input type="number" class="form-control rounded-10" name="total_discount" id="total_discount"
                        value="{{ optional($quotation)->total_discount }}" onkeyup="updateTotal()">
                </div>
        ...
        <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-dark px-4 text-uppercase rounded-10 submitForm"
                data-method="adminedit" data-form="quotationForm" data-target="{{ url('quotation') }}"
                ...
                success: function(response) {
                    if (response.status === 1) {
                        ...
                        if ($("#customer_id").length) {
                            ...
                            if ($("#customer_id option[value='" + response.id + "']")
                                .length === 0) {
                                const newOption = new Option(response.name, response.id,
                                    false, true);
        ...
        fetch(actionUrl, {
            ...
            .then data => {
                ...
                if (data.status === 1) {
                    ...
                    newOption.setAttribute("data-image", data.image ? data.image :
    ...
    $(".addITem").on("change", function() {
        ...
        if (item.val() != '' && found) {
            ...
            let dynamicRowHTML = `
                ...
                unit_price + `" id="unit_price` + rand + `" onfocus="unblurCostPrice('unit_price` + rand +
...
<script>
    $(document).ready(function() {
        ...
        $('#item_name').on('change', function() {
            ...
            let url =
            "{{ url('admin/item-by-barcode') }}"; // Define the route for fetching item details
